...
class WandbLogger:
    ...
    def log_checkpoint(self, current_epoch, current_step):
        ...
        gen_path = os.path.join(self.config.path["checkpoint"], f"I{current_step}_E{current_epoch}_gen.pth")
        opt_path = os.path.join(self.config.path["checkpoint"], f"I{current_step}_E{current_epoch}_opt.pth")
    ...
    def log_eval_data(self, fake_img, sr_img, hr_img, psnr=None, ssim=None):
        ...
        if psnr is not None and ssim is not None:
            self.eval_table.add_data(
                self._wandb.Image(fake_img), self._wandb.Image(sr_img), self._wandb.Image(hr_img), psnr, ssim
            )
        else:
